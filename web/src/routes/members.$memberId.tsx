...
function MemberDetailPage() {
  ...
  const relatedMembers = useMemo(() => {
    ...
    return members
      .filter((item) => item.id !== member.id && getMemberClassName(item) === currentClassName)
      .sort((a, b) => stableScore(member.id, a.id) - stableScore(member.id, b.id))
      .slice(0, 4)
  }, [member, members])
  ...
  if (error || !member) {
    return <NotFoundState title="Không tìm thấy thành viên" to="/members" action="Về danh sách bạn bè" />
  }
          ...
          <div className="grid grid-cols-1 gap-7 lg:grid-cols-12 lg:items-center">
            <div className="lg:col-span-5">
              <div className="overflow-hidden rounded-lg border-4 border-white bg-slate-100 shadow-xl md:border-8 lg:max-h-[72svh]">
                {thumbUrl ? (
                  <img src={thumbUrl} alt={member.name} className="aspect-[4/5] h-full w-full object-cover" />
                ) : (
      ...
      <section className="border-t border-reunion-gold/10 bg-reunion-paper py-10 md:py-12">
        <div className="section-container">
          <div className="mb-8 space-y-3">
            ...
            <h2 className="font-serif text-3xl font-bold text-reunion-ink md:text-4xl">Bạn cùng lớp</h2>
          </div>
          ...
          {isLoadingMembers ? (
            ...
            <div className="rounded-lg border border-dashed border-slate-200 bg-white/70 p-7 text-center">
              ...
              <p className="font-serif italic text-slate-400">Chưa có thành viên cùng lớp khác.</p>
            </div>
          )}
        </div>
      </section>
...
function NotFoundState({ title, to, action }: { title: string; to: '/members'; action: string }) {
  return (
    <div className="section-container py-14 text-center">
      <div className="mx-auto max-w-md space-y-4 rounded-lg border border-dashed border-slate-200 bg-white/70 p-7">
        ...
        <h1 className="font-serif text-2xl font-bold text-reunion-ink">{title}</h1>
